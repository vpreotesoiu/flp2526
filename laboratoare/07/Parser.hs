...
import Syntax ( Expr(..), Variable(..), Type(..), Equation(..))
import Substitution (SubstitutionLike(..))
...
-- | Like 'spaces' but expects at least a space-like character
spaces1 :: Parser String
spaces1 = some (sat isSpace)
...
{-| sepByN behaves like 'sepBy0' and 'sepBy1', but expects at least n occurrences of p, otherwise it fails

>>> parse (sepByN 2 digit (symbol ",")) "1,2,3"
[([1,2,3],""),([1,2],",3")]
-}
sepByN :: Int -> Parser a1 -> Parser a2 -> Parser [a1]
sepByN 0 p sep = sepBy0 p sep
...
indexedVar :: Parser Variable
indexedVar = V <$> identifier <*> (char '_' *> integerToken)
...
{- | a parser for applications (left-associative)
Examples:

>>> parseFirst papp "abc  d"
Right (abc d)

>>> parseFirst pvar "f ()"
Left "No parse consuming all input"

>>> parseFirst papp "a b c d"
Right (((a b) c) d)
-}
papp :: Parser Expr
papp = foldl1 App <$> sepByN 2 efactor (sat isSpace)
...
-}
pabs :: Parser Expr
pabs = mkLambdas <$> (symbol "\\" *> sepBy1 pvar (sat isSpace)) <*> (symbol "->" *> pexpr)
  where
    mkLambdas = flip (foldr Lambda)

{- | A parser for lambda-terms

Examples:

>>> parseFirst pexpr "(\\x -> x) x (y z) (r t)"
Right ((((\ x -> x) x) (y z)) (r t))
-}
pexpr :: Parser Expr
...
pequation = (:=:) <$> ptype <*> (symbol "=" *> ptype)

pequations :: Parser [Equation]
pequations = commaSep1 pequation

psubSingle :: SubstitutionLike s => Parser s
psubSingle = singletonSubst <$> pvar <*> (symbol "|->" *> ptype)

psubst :: SubstitutionLike s => Parser s
psubst = foldr (>>>) idSubst <$> commaSep0 psubSingle
