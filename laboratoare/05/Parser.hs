...
import Syntax ( Expr(..), Equation(..), Clause(..), Program(..), Query(..), Variable(..) )
...
indexedVar = V <$> variable <*> (char '_' *> integerToken)
...
psubSingle = singletonSubst <$> pvar <*> (symbol "|->" *> pexpr)
